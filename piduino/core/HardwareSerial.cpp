...
volatile char uart_rx_buffer[1024];
volatile char uart_tx_buffer[1024];
...
void uart_check_fifos(){
	if((UART0CR & _BV(UART0EN)) != 0){
		while((UART0FR & _BV(UART0RXFE)) == 0 && uart_rx_buffer_index < 1024) uart_rx_buffer[uart_rx_buffer_index++] = UART0DR;
...
size_t HardwareSerial::write(uint8_t c){
	if(uart_tx_buffer_index < 1024) uart_tx_buffer[uart_tx_buffer_index++] = c;
  return 1;
}
